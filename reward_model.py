...
from prompt import (
    ...
    gpt_free_query_env_prompts, gpt_summary_env_prompts, gpt_prefix_env_prompts
)
...
from utils import jsd_divergence
...
class RewardModel:
    def __init__(self, ds, da, 
        ...
        self.buffer_rew1 = np.empty((self.capacity, size_segment, 1), dtype=np.float32)
        self.buffer_rew2 = np.empty((self.capacity, size_segment, 1), dtype=np.float32)
        ...
        if cached_label_path is None or cached_label_path == 'None':
    ...
    def construct_ensemble(self):
        for i in range(self.de):
            if not self.image_reward:
                d_input = self.ds+self.da if self.include_action else self.ds
                model = nn.Sequential(*gen_net(in_size=d_input, 
    ...
    def add_data(self, obs, act, rew, done, info=None):
        ...
        text = None
        if info is not None:
            if 'img' in info.keys() and info['img'] is not None:
                img = info['img']
                flat_img = img.reshape(1, img.shape[0], img.shape[1], img.shape[2])
            
            if 'text' in info.keys() and info['text'] is not None:
                text = info['text']
        ...
        if init_data:
            ...
            if 'Cloth' not in self.env_name:
                ...
                if text is not None:
                    self.text_inputs[-1].append(text)
    ...
    def get_queries(self, mb_size=20):
        ...
        if self.llm_label:
            train_texts = self.text_inputs[:max_len]
        ...
        if self.llm_label:
            text_t_2 = [train_texts[i] for i in batch_index_2]
        ...
        if self.llm_label:
            text_t_1 = [train_texts[i] for i in batch_index_1]
        ...
        if self.llm_label:
            text_t_1 = [t for seg in text_t_1 for t in seg]# sum(text_t_1, [])
            text_t_2 = [t for seg in text_t_2 for t in seg]# sum(text_t_2, [])
        ...
        if self.llm_label:
            if self.vlm_label and not self.vlm_label == self.llm_label:
                raise ValueError('VLM label and LLM label are both set and not equal.')
            if self.llm_label == 1:
                text_time_idex = np.array([[i*len_traj+self.size_segment - 1] for i in range(mb_size)])
            else:
                raise NotImplementedError
            
            if 'Cloth' not in self.env_name:
                text_time_index_2 = text_time_idex + random_idx_2
                text_time_index_1 = text_time_idex + random_idx_1
            else:
                raise NotImplementedError
                # image_time_index_2 = image_time_index
                # image_time_index_1 = image_time_index
        ...
        if self.llm_label:
            text_t_1 = np.take(text_t_1, text_time_index_1, axis = 0) # Batch x llm_label x 1(sentence)
            text_t_2 = np.take(text_t_2, text_time_index_2, axis = 0)
        ...
        if not self.vlm_label and not self.image_reward:
            if not self.llm_label:
                return sa_t_1, sa_t_2, r_t_1, r_t_2
            else:
                return sa_t_1, sa_t_2, r_t_1, r_t_2, text_t_1, text_t_2
        elif self.vlm_label or self.image_reward:
            if not self.llm_label:
                return sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1, img_t_2
            else:
                return sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1, img_t_2, text_t_1, text_t_2
        # elif self.llm_label and not self.vlm_label and not self.image_reward:
        #     return sa_t_1, sa_t_2, r_t_1, r_t_2, text_t_1, text_t_2
        # elif self.llm_label and self.:


    def put_queries(self, sa_t_1, sa_t_2, labels, rew1 = None, rew2 = None):
        ...
        if next_index >= self.capacity:
            ...
            if rew1 is not None:
                np.copyto(self.buffer_rew1[self.buffer_index:self.capacity], rew1[:maximum_index])
                np.copyto(self.buffer_rew2[self.buffer_index:self.capacity], rew2[:maximum_index])
            ...
            if remain > 0:
                ...
                if rew1 is not None:
                    np.copyto(self.buffer_rew1[0:remain], rew1[maximum_index:])
                    np.copyto(self.buffer_rew2[0:remain], rew2[maximum_index:])
            ...
            if rew1 is not None:
                np.copyto(self.buffer_rew1[self.buffer_index:next_index], rew1)
                np.copyto(self.buffer_rew2[self.buffer_index:next_index], rew2)

            self.buffer_index = next_index
            
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1=None, img_t_2=None, text_t_1=None, text_t_2=None):
        ...
        if self.vlm_label or self.llm_label:
            ts = time.time()
            ...
            elif self.vlm == 'text_only':
                for idx, (sa1, sa2, text1, text2) in enumerate(zip(sa_t_1, sa_t_2, text_t_1, text_t_2)):
                    combined = f'Scene 1: {text1}.\nScene 2: {text2}.'
                    combined_images_list.append(combined)
                    diff = np.linalg.norm(sa1 - sa2)
                    if diff < 1e-3:
                        useful_indices.append(0)
                    else:
                        useful_indices.append(1)
            ...
            if self.vlm == 'gpt4v_two_image': 
                ...
                for idx, (img_path_1, img_path_2) in enumerate(gpt_two_image_paths):
                    ...
                    prefix_prompt = gpt_prefix_env_prompts[self.env_name]
                ...
                from vlms.gpt4_infer import gpt4_infer_text_only
                vlm_labels = []
                for idx, (text_1, text_2) in enumerate(zip(text_t_1, text_t_2)):
                    query_prompt = gpt_free_query_env_prompts[self.env_name]
                    summary_prompt = gpt_summary_env_prompts[self.env_name]
                    prefix_prompt = gpt_prefix_env_prompts[self.env_name]
                    res = gpt4_infer_text_only(query_prompt, summary_prompt, text_1, text_2, prefix_prompt=prefix_prompt, 
                        save_path=os.path.join(self.log_dir, 'model_responses.txt'), save_chance=0.05)
                    try:
                        label_res = int(res)
                    except:
                        label_res = -1
            ...
            elif self.vlm == 'gpt4v_text_and_img':
                raise NotImplementedError
            ...
            if self.llm_label:
                text_t_1 = text_t_1[good_idx]
                text_t_2 = text_t_2[good_idx]
    ...
    def uniform_sampling(self):
        if not self.vlm_label and not self.llm_label: 
            # get queries
            if not self.image_reward:
                sa_t_1, sa_t_2, r_t_1, r_t_2 =  self.get_queries(
            ...
            if self.cached_label_path is None:
                if self.vlm_label and not self.llm_label:
                    sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1, img_t_2 =  self.get_queries(
                        mb_size=self.mb_size)
                    text_t_1, text_t_2 = None, None
                elif not self.vlm_label and self.llm_label:
                    sa_t_1, sa_t_2, r_t_1, r_t_2, text_t_1, text_t_2 =  self.get_queries(
                        mb_size=self.mb_size)
                    img_t_1, img_t_2 = None, None
                elif self.vlm_label and self.llm_label:
                    sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1, img_t_2, text_t_1, text_t_2 =  self.get_queries(
                        mb_size=self.mb_size)

                if not self.image_reward:
                    sa_t_1, sa_t_2, r_t_1, r_t_2, gt_labels, vlm_labels = self.get_label(
                        sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1, img_t_2, text_t_1, text_t_2)
                else:
        ...
        if len(labels) > 0:
            if not self.image_reward:
                self.put_queries(sa_t_1, sa_t_2, labels, rew1=r_t_1, rew2=r_t_2)
            else:
                self.put_queries(img_t_1[:, ::self.resize_factor, ::self.resize_factor, :], img_t_2[:, ::self.resize_factor, ::self.resize_factor, :], labels, rew1=r_t_1, rew2=r_t_2)
    ...
    def train_reward(self):
        ...
        ensemble_acc = np.array([0 for _ in range(self.de)])
        ensemble_rational_acc = np.array([0 for _ in range(self.de)])
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                rew1 = np.sum(self.buffer_rew1[idxs], axis = 1)
                rew2 = np.sum(self.buffer_rew2[idxs], axis = 1)
                rew1 = torch.from_numpy(rew1).to(device)
                rew2 = torch.from_numpy(rew2).to(device)
                rational_labels = 1*(rew1 < rew2)
                rational_labels = rational_labels.squeeze()
                ...
                # compute loss
                valid_label_count = (labels!=-1).sum()
                curr_loss = self.CEloss(r_hat, labels)
                curr_loss = curr_loss.sum() / valid_label_count
                ...
                rational_correct = (predicted == rational_labels).sum().item()
                ensemble_rational_acc[member] += rational_correct
                
            loss.backward()
            self.opt.step()
        
        ensemble_acc = ensemble_acc / total
        ensemble_rational_acc = ensemble_rational_acc / total

        torch.cuda.empty_cache()
        
        return ensemble_acc, ensemble_rational_acc
    ...
    def train_confidence_reward(self):
        ensemble_losses = [[] for _ in range(self.de)]
        ensemble_acc = np.array([0 for _ in range(self.de)])
        ensemble_rational_acc = np.array([0 for _ in range(self.de)])
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                ## compute weights
                uncertain_index = labels == -1
                valid_label_count = (labels!=-1).sum()
                tmp_labels = labels.float()
                tmp_labels[uncertain_index] = 0.
                proxy_rews = torch.cat([rew1, rew2], axis=-1)
                proxy_pref = torch.softmax(proxy_rews, dim=1)
                weights = jsd_divergence(proxy_pref[:,0], tmp_labels)
                weights[uncertain_index] = 0.
                # label_dist = Bernoulli(proxy_pref[:,0])
                # weights = kl_divergence()
                ...
                curr_loss = (curr_loss * weights).sum() / valid_label_count
                # curr_loss = curr_loss.mean()
                loss += curr_loss
                ensemble_losses[member].append(curr_loss.item())
