...
class Workspace(object):
    def __init__(self, cfg):
        ...
        wandb.init(
            project = "rlaif",
            name = f"rlvlmf_{cfg.env}_seed_{cfg.seed}",
            config = vars(cfg),
        )
    ...
    def evaluate(self, save_additional=False):
        ...
        for key,value in extra.items():
            wandb_log_dict.update({f"eval/{key}": value})
        if self.log_success:
            wandb_log_dict.update({"eval/success_rate": success_rate, "train/true_episode_success": success_rate})


        wandb.log(wandb_log_dict, self.step)
        ...
        while self.step < self.cfg.num_train_steps:
            if done:
                if self.step > 0:
                    ...
                    for key, value in extra.items():
                        wandb_log_dict.update({f"train/{key}": value})
                    wandb.log(wandb_log_dict, self.step)
                ...
                if self.step > 0 and self.step % self.cfg.eval_frequency == 0:
                    ...
                    wandb.log({"eval/episode": episode}, self.step)
                ...
                if self.log_success:
                    wandb_log_dict.update({
                        "train/episode_success": episode_success,
                        "train/true_episode_success": episode_success,
                    })
                ...
                wandb.log({"train/episode": episode}, self.step)
